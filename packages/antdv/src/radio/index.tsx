...
const TransformRadioGroup = transformComponent<RadioGroupProps>(AntdRadioGroup, {
  change: 'input',
})
              ...
              default: () =>
                options.map(option => {
                  if (typeof option === 'string' || typeof option === 'number') {
                    return <Radio value={option}>{resolveComponent(slots?.option ?? option, { option })}</Radio>
                  } else {
                    return (
...
const RadioGroup = connect(
  itemRender(RadioGroupOption),
  mapProps({ dataSource: 'options' }),
  mapReadPretty(PreviewText.Select)
)

export const Radio = Object.assign(itemRender(AntdRadio), {
  Group: RadioGroup,
})
