import type { ArrayField, FieldDisplayTypes, GeneralField } from '@formvk/core'
import type { Schema } from '@formvk/json-schema'
import { useObserver } from '@formvk/reactive-vue'
import { FormPath, isArr, isBool, isEmpty } from '@formvk/shared'
...
import { FieldContext, usePrefixCls } from '../__builtins__'
...
const useArrayTableSources = (arrayFieldRef: Ref<ArrayField>, schemaRef: Ref<Schema>) => {
  ...
  const parseSources = (schema: Schema): ObservableColumnSource[] => {
    if (isColumnComponent(schema) || isOperationsComponent(schema) || isAdditionComponent(schema)) {
      ...
      const title = field?.title || schema.title
      const columnProps = field?.componentProps || schema['x-component-props'] || {}
...
const useArrayTableColumns = (dataSource: any[], sources: ObservableColumnSource[]): ColumnProps[] => {
  return sources.reduce<ColumnProps[]>((buf, { name, columnProps, schema, display, field }, key) => {
    ...
    return buf.concat({
      ...
      customRender: ({ record }) => {
        ...
        if (!schema.properties) {
          const value = FormPath.getIn(record, name)
          if (isEmpty(value)) return <div>-</div>
...
const ArrayTablePagination = defineComponent<IArrayTablePaginationProps>({
  ...
  setup(_props, { attrs, slots }) {
    ...
    return () => {
      ...
      return <>{slots.default?.(dataSource.value?.slice(startIndex.value, endIndex.value + 1), renderPagination)}</>
    }
  },
})

const ArrayTableInner = defineComponent(
  (_: TableProps, { attrs, slots }) => {
    useObserver()
    const fieldRef = useField<ArrayField>()
...
export const ArrayTable = Object.assign(ArrayTableInner, {
