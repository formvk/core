import { DeleteOutlined, DownOutlined, MenuOutlined, PlusOutlined, UpOutlined } from '@ant-design/icons-vue'
import type { ArrayField } from '@formvk/core'
import { clone, isValid, uid } from '@formvk/shared'
import type { Schema } from '@formvk/vue'
import { useField, useFieldSchema } from '@formvk/vue'
...
const ArrayBaseSortHandle = defineComponent({
  ...
  setup(_props, { attrs }) {
    ...
    return () => {
      ...
      return wrapSSR(
        withDirectives(<MenuOutlined class={`${prefixCls}-sort-handle ${hashId.value}`} {...attrs} />, [
          [HandleDirective],
        ])
      )
    }
...
const ArrayBaseRemove = defineComponent({
  ...
  setup(props, { attrs, emit }) {
    ...
    return () => {
      ...
      return wrapSSR(<DeleteOutlined {...attrs} class={`${prefixCls}-remove ${hashId.value}`} onClick={onClick} />)
    }
  },
})
