...
export const genCommonStyle = (token: ComputedRef<GlobalToken>, componentPrefixCls: string): CSSObject => {
  const { fontFamily, fontSize } = token.value
    ...
    return [
      useStyleRegister(info, () => {
        ...
        const mergedToken: TokenWithCommonCls<GlobalToken> = merge(token.value, {
          ...token.value['Form'], // Merge the antd form token
          componentCls,
          prefixCls,
