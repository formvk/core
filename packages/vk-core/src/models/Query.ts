...
export class Query {
  ...
  constructor(props: IQueryProps) {
    ...
    if (!this.pattern.isMatchPattern) {
      const matched = takeMatchPattern(
        this.form,
        this.pattern.haveRelativePattern ? buildDataPath(props.form.fields, this.pattern) : this.pattern
      )
      if (matched) {
  ...
  forEach<Result>(iterator: (field: GeneralField, address: FormPath) => Result) {
