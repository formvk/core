...
const isNumberIndex = (val: any) => (isStr(val) ? /^\d+$/.test(val) : isNum(val))
...
export class Path {
  ...
  transform = <T>(regexp: string | RegExp, callback: (...args: string[]) => T): T | string => {
    ...
    const args = this.segments.filter(key => reg.test(key as string)) as string[]
  ...
  match = (pattern: Pattern): boolean => {
    ...
    if (path.isMatchPattern) {
      ...
      if (this.isMatchPattern) {
        ...
        const result = cacheWith(new Matcher(this.tree, record).match(path.segments))
        ...
        const result = cacheWith(Matcher.matchSegments(this.segments, path.segments, record))
        this.matchScore = record.score
        return result.matched
      }
    }
  ...
  static isPathPattern(target: any): target is Pattern {
    return !!(isStr(target) || isArr(target) || isRegExp(target) || (isFn(target) && target[isMatcher]))
  }
