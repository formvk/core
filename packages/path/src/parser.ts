...
const calculate = (a: string | number, b: string | number, operator: string) => {
...
export class Parser extends Tokenizer {
  ...
  parseAtom(type?: Token): Node | undefined {
    switch (type) {
      case braceLTok:
      case bracketLTok:
  ...
  parseDestructorExpression(): DestructorExpressionNode {
    ...
    node.source = this.input
      ...
      .replace(/\[\s*([\\+\-\\*\\/])?\s*([^,\]\s]*)\s*\]/, (match, operator, target) => {
  ...
  parseArrayPattern(): ArrayPatternNode {
    ...
    this.next()
    node.elements = this.parseArrayPatternElements() as ObjectPatternNode[] | ArrayPatternNode[] | IdentifierNode[]
    return node
  }
  ...
  parseObjectProperties(): ObjectPatternPropertyNode[] {
    const nodes: ObjectPatternPropertyNode[] = []
    while (this.state.type !== braceRTok && this.state.type !== eofTok) {
      ...
      if (this.state.type === colonTok) {
        this.next()
        node.value = this.parseAtom(this.state.type) as IdentifierNode | ObjectPatternNode[] | ArrayPatternNode[]
      }
