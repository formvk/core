...
const match = (obj: Record<string, any>) => {
...
const unMatch = (obj: Record<string, any>) => {
  for (const name in obj) {
...
test('test matchGroup', () => {
  ...
  expect(excludePattern.matchAliasGroup('aa.bb.11.mm', 'aa.cc.dd.bb.11.mm')).toEqual(true)
  expect(excludePattern.matchAliasGroup('aa.cc', 'aa.kk.cc')).toEqual(false)
  expect(new Path('aa.*(!bb)').matchAliasGroup('kk.mm.aa.bb', 'aa.bb')).toEqual(false)
  ...
  expect(new Path('aa.*(!bb)').matchAliasGroup('kk.mm.aa.cc', 'aa.cc')).toEqual(true)
...
test('exclude match', () => {
  ...
  expect(Path.parse('*(!basic.name,versionTag)').match('isExecutable')).toBeTruthy()
  expect(Path.parse('*(!basic.name,versionTag)').match('versionTag')).toBeFalsy()
  expect(Path.parse('*(!basic.name,basic.name.*,versionTag)').match('basic.name')).toBeFalsy()
  expect(Path.parse('*(!basic.name,basic.name.*,versionTag)').match('basic.name.kkk')).toBeFalsy()
...
test('test optional wild match', () => {
  ...
  expect(Path.parse('*(aa.**,bb.**).bb').match(['aa', 'oo', 'bb'])).toEqual(true)
  expect(Path.parse('*(aa.**,bb.**).bb').match(['bb', 'oo', 'bb'])).toEqual(true)
  expect(Path.parse('*(aa.**,bb.**).bb').match(['aa', 'oo', 'kk', 'dd', 'bb'])).toEqual(true)
  expect(Path.parse('*(aa.**,bb.**).bb').match(['cc', 'oo', 'kk', 'dd', 'bb'])).toEqual(false)
  expect(Path.parse('*(aa.**,bb.**).bb').match(['bb', 'oo', 'kk', 'dd', 'bb'])).toEqual(true)
  expect(Path.parse('*(aa.**,bb.**).bb').match(['kk', 'oo', 'kk', 'dd', 'bb'])).toEqual(false)
})

test('test expand', () => {
  expect(Path.parse('t.0.value~').match(['t', 0, 'value_list', 'hello'])).toEqual(false)
})
...
test('nested group match', () => {
  expect(Path.parse('aa.*.*(bb,cc).dd.*(kk,oo).ee').match('aa.0.cc.dd.kk.ee')).toEqual(true)
})

test('group match with destructor', () => {
  expect(Path.parse('*([startDate,endDate],date,weak)').match('date')).toEqual(true)
  expect(Path.parse('*({startDate,endDate},date,weak)').match('date')).toEqual(true)
  expect(Path.parse('*([startDate,endDate],date,weak)').match('xxx')).toEqual(false)
  expect(Path.parse('*({startDate,endDate},date,weak)').match('xxx')).toEqual(false)
  expect(Path.parse('*([startDate,endDate],date,weak)').match('[startDate,endDate]')).toEqual(true)
  expect(Path.parse('*({startDate,endDate},date,weak)').match('{startDate,endDate}')).toEqual(true)
})

test('all range match', () => {
  expect(Path.parse('array.*[:].*[:].*[:].bb').match('array.0.0.0.aa')).toBeFalsy()
})
