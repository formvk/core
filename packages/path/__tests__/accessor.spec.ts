...
test('getIn with a.b.c.{aaa,bbb}', () => {
  expect(Path.getIn({ a: { b: { c: { aaa: 123, bbb: 321 } } } }, 'a.b.c.{aaa,bbb}')).toEqual({ aaa: 123, bbb: 321 })
})
...
test('getIn with a.b.c.{aaa,bbb} source has extra property', () => {
  expect(Path.getIn({ a: { b: { c: { aaa: 123, bbb: 321, kkk: 'ddd' } } } }, 'a.b.c.{aaa,bbb}')).toEqual({
    aaa: 123,
    bbb: 321,
  })
})
...
test('getIn with a.b.c.{aaa:ooo,bbb}', () => {
  expect(Path.getIn({ a: { b: { c: { ooo: 123, bbb: 321, kkk: 'ddd' } } } }, 'a.b.c.{aaa:ooo,bbb}')).toEqual({
    aaa: 123,
    bbb: 321,
  })
})
...
test('getIn with a.b.c.[aaa,bbb]', () => {
  expect(Path.getIn({ a: { b: { c: { aaa: 123, bbb: 321 } } } }, 'a.b.c.[aaa,bbb]')).toEqual([123, 321])
})

test('setIn with a.b.c.[aaa,bbb] source has extra property', () => {
  expect(Path.setIn({ a: { b: { c: { kkk: 'ddd' } } } }, 'a.b.c.[aaa,bbb]', [123, 321])).toEqual({
    a: { b: { c: { aaa: 123, bbb: 321, kkk: 'ddd' } } },
  })
})

test('getIn with a.b.c.[aaa,bbb] source has extra property', () => {
  expect(Path.getIn({ a: { b: { c: { aaa: 123, bbb: 321, kkk: 'ddd' } } } }, 'a.b.c.[aaa,bbb]')).toEqual([123, 321])
})

test('setIn with a.b.c.[{ddd,kkk:mmm},bbb]', () => {
  expect(Path.setIn({}, 'a.b.c.[{ddd,kkk:mmm},bbb]', [{ ddd: 123, kkk: 'hhh' }, 321])).toEqual({
    a: { b: { c: { ddd: 123, bbb: 321, mmm: 'hhh' } } },
  })
})

test('getIn with a.b.c.[{ddd,kkk:mmm},bbb]', () => {
  expect(Path.getIn({ a: { b: { c: { ddd: 123, bbb: 321, mmm: 'hhh' } } } }, 'a.b.c.[{ddd,kkk:mmm},bbb]')).toEqual([
    { ddd: 123, kkk: 'hhh' },
    321,
  ])
})
...
test('existIn with a.b.c', () => {
  ...
  expect(
    Path.existIn({ a: { b: { c: { ooo: 123, ccc: 123, ddd: 321, kkk: 'ddd' } } } }, 'a.b.c.{aaa:ooo,bbb:[ccc,ddd]}')
  ).toEqual(true)
  expect(
    Path.existIn({ a: { b: { c: { ooo: 123, ccc: 123, kkk: 'ddd' } } } }, 'a.b.c.{aaa:ooo,bbb:[ccc,ddd]}')
  ).toEqual(false)
...
test('deleteIn', () => {
  expect(Path.deleteIn({ a: { b: { c: { ooo: 123, ccc: 234 } } } }, 'a.b.c.ccc')).toEqual({
    a: { b: { c: { ooo: 123 } } },
  })

  expect(Path.deleteIn({ a: { b: { c: { ooo: 123, ccc: 234 } } } }, null as any)).toEqual({
    a: { b: { c: { ooo: 123, ccc: 234 } } },
  })

  expect(Path.deleteIn({ a: { b: { c: { ooo: 123, ccc: 234 } } } }, [])).toEqual({
    a: { b: { c: { ooo: 123, ccc: 234 } } },
  })
...
test('ensureIn', () => {
  ...
  expect(Path.parse('a.b').ensureIn({ a: { b: 'value' } }, 'default')).toEqual('value')
  expect(Path.ensureIn({}, 'a.b.c', 'default')).toEqual('default')
})

test('complex destructing', () => {
  expect(
    Path.setIn({}, '{aa:{bb:{cc:destructor1,dd:[destructor2,destructor3],ee}}}', {
      aa: {
        bb: {
          cc: 123,
          dd: [333, 444],
...
test('path arguments', () => {
  ...
  expect(new Path(undefined as any).segments).toEqual([])
})

test('path methods', () => {
  const path = Path.parse('a.b.c')

  expect(path.concat(Path.parse('d.e')).segments).toEqual(['a', 'b', 'c', 'd', 'e'])
  ...
  expect(Path.parse('a.b.c').transform(/[a-z]/, (...result) => result)).toEqual(['a', 'b', 'c'])
  expect(Path.parse('a.b.c').transform(/[a-b]/, (...result) => result)).toEqual(['a', 'b'])
  expect(Path.parse('a.b.c').transform('', null as any)).toEqual('')
  expect(() => Path.parse('*').transform('', () => {})).toThrowError()
  expect(Path.transform('a.b.c', /[a-z]/, (...result) => result)).toEqual(['a', 'b', 'c'])
