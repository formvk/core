...
import { each, FormPath, FormPathPattern, isFn, isValid, toArr } from '@formvk/shared'
import { destroy, getArrayParent, getObjectParent, initFieldUpdate, locateNode } from '../shared/internals'
import {
  DisplayTypes,
  FieldComponent,
  FieldDecorator,
...
export class BaseField<Decorator = any, Component = any, TextType = any> {
  ...
  get pattern(): PatternTypes {
    const parentPattern: PatternTypes = this.parent?.pattern || this.form.pattern || 'editable'
  ...
  set pattern(pattern: PatternTypes) {
  ...
  setDisplay = (type: DisplayTypes) => {
    this.display = type
  }

  setPattern = (type: PatternTypes) => {
    this.pattern = type
  }
