import { createForm, Form, IFormProps, onFormSubmitSuccess } from '@formvk/core'
import { FormProvider, observer, ReactFC } from '@formvk/react'
import { toJS } from '@formvk/reactive'
import { applyMiddleware, IMiddleware, isBool, isFn, isNum, isStr } from '@formvk/shared'
...
const DrawerExtra: ReactFC = props => {
  ...
  useLayoutEffect(() => {
    ...
    if (content) {
      if (!extraRef.current) {
        extraRef.current = content.querySelector(`.${prefixCls}-extra`) as HTMLDivElement
        if (!extraRef.current) {
